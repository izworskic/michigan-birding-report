...
const MICHIGAN_DATE = new Intl.DateTimeFormat('en-US', {
...
function freshnessLabel(obsDt, now = new Date()) {
  if (!obsDt) return 'recent';
  const match = String(obsDt).match(/^(\d{4})-(\d{2})-(\d{2})/);
  if (!match) return String(obsDt);

  const current = dateParts(now);
  const observationDay = dateSerial(Number(match[1]), Number(match[2]), Number(match[3]));
  const currentDay = dateSerial(current.year, current.month, current.day);
  const diffDays = Math.max(0, currentDay - observationDay);
...
function summarizeObservations(observations = [], now = new Date()) {
  ...
  return {
    ...
    freshness: freshnessLabel(latest, now),
